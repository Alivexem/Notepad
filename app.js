...
save.addEventListener("click", () => {
    ...
    let existingNoteIndex = savedNotes.findIndex((note) =>
        note.title === title
    );
...
function addNoteToDOM(title, main, createdAt) {
    ...
    recordText.innerText = `Created on: ${
    ...
    deleteIcon.addEventListener("click", (e) => {
        let decision = confirm(
            `Are you sure you want to delete ${title} note?`,
        );

        if (decision) {
            let savedNotes = JSON.parse(localStorage.getItem("notes")) || [];
            let noteIndex = savedNotes.findIndex((note) =>
                note.title === title
            );
